...
def create_folder(dir: str):
    ...
    with open(dir+'/'+alt_name+'.txt', 'w') as file: pass
    ...
    if x.lower() == 'y':
        ...
        create_init_lcs(dir, empty = True)

        create_init_ld(dir, empty = True)

        print('Once you have these, run the function create_init_lcs and then create_init_ld.')
    ...
    if x.lower() == 'y':
        ...
        create_init_rv(dir, empty = True)

        print('Once you have these, run the function create_init_rv.')

    
    create_init_star(dir)

    create_init_planets(dir)


def create_init_lcs(dir: str, empty = False):
    ...
    tab = Table(rows = rows,
            names = ['File','Nickname','Time Col','Flux Col','Err Col','Quality Col','Time Offset','Err Scale','Exp Time','Filter'],
            units = [None,None,None,None,None,None,'to BJD',None,'s',None],
            dtype = [str,str,str,str,str,str,int,float,float,str])
    
    tab.write(dir+'/init_lcs.txt', format = 'ascii.fixed_width_two_line', overwrite = True, delimiter = '|', delimiter_pad = ' ', bookend = True,
          formats = {'File': '%-75s', 'Nickname': '%10s', 'Time Col': '%10s', 'Flux Col': '%15s', 'Err Col': '%20s', 'Quality Col': '%10s', 'Time Offset': '%7i', 'Err Scale': '%8.4f', 'Exp Time': '%7.2f', 'Filter': '%10s'},
          header_rows = ['name', 'unit'])


def create_init_rv(dir: str, empty = False):
    ...
    tab = Table(rows = rows,
            names = ['File','Nickname','Time Col','RV Col','Err Col','Err Scale','m/s or km/s'],
            dtype = [str,str,str,str,str,float,str])
    
    tab.write(dir+'/init_rv.txt', format = 'ascii.fixed_width_two_line', overwrite = True, delimiter = '|', delimiter_pad = ' ', bookend = True,
          formats = {'File': '%-20s', 'Nickname': '%10s', 'Time Col': '%10s', 'RV Col': '%15s', 'Err Col': '%20s', 'Err Scale': '%8.4f', 'm/s or km/s': '%4s'},
          header_rows = ['name'])


def create_init_star(dir: str):
    ...
    tab.write(dir+'/init_star.txt', format = 'ascii.fixed_width_two_line', overwrite = True, delimiter = '|', delimiter_pad = ' ', bookend = True,
          formats = {'Radius': '%6.3f', 'Mass': '%6.3f', 'Teff': '%5i', 'Parallax': '%8.4f', 'log(g)': '%5.3f', 'Fe/H': '%6.3f', 'J': '%6.3f','H': '%6.3f','K': '%6.3f','G': '%7.4f','Bp': '%7.4f','Rp': '%7.4f','W1': '%6.3f','W2': '%6.3f','W3': '%6.3f'},
          header_rows = ['name', 'unit'])


def create_init_ld(dir: str, empty = False):
    ...
    if empty:
        ...
        while True:
            ...
            x = input('More filters to do (y/n)? Make sure to do one set of limb darkening for each filter used in the light curves.')
    ...
    tab.write(dir+'/init_ld.txt', format = 'ascii.fixed_width_two_line', overwrite = True, delimiter = '|', delimiter_pad = ' ', bookend = True,
          formats = {'u1': '%6.4f', 'u2': '%6.4f', 'Filter': '%10s'},
          header_rows = ['name'])


def create_init_planets(dir: str):

    rows = []

    n = int(input('How many planets are in the system? '))

    for i in range(n):

        row = []

        transit = input('Is planet {0} transitting? True or False '.format(i+1))
        row.append(transit)

        rv = input('Does planet {0} have an RV signal? True or False '.format(i+1))
        row.append(rv)

        if transit == 'True':

            ttv = input('Fit planet {0} for ttvs? True or False '.format(i+1))
            row.append(ttv)
        ...
        ecc = input('Fit planet {0} for eccentricity? True or False '.format(i+1))
        row.append(ecc)

        p = input('Planet {0} period initial guess (in days): '.format(i+1))
        row.append(p)

        tc = input('Planet {0} tc initial guess (in BJD - 2450000): '.format(i+1))
        row.append(tc)

        if transit == 'True':

            rp = input('Planet {0} to star radius ratio initial guess: '.format(i+1))
            row.append(rp)

            a = input('Planet {0} semimajor axis to stellar radius ratio initial guess: '.format(i+1))
            row.append(a)

            cosi = input('Planet {0} cos(inclination) initial guess: '.format(i+1))
            row.append(cosi)
        ...
        if rv == 'True':
            ...
            k = input('Planet {0} rv semi-amplitude initial guess (in m/s): '.format(i+1))
            row.append(k)
        ...
        if rv == 'True' and ecc == 'True':

            secw = input('Planet {0} sqrt(e)cos(w) initial guess: '.format(i+1))
            row.append(secw)

            sesw = input('Planet {0} sqrt(e)sin(w) initial guess: '.format(i+1))
            row.append(sesw)
    ...
    tab = Table(rows = rows, names = ['Transiting', 'RV Signal', 'Fit TTVs', 'Fit Ecc', 'Period', 'Tc', 'Rp/Rs', 'a/Rs', 'cos(i)', 'K', 'sqrt(e)cos(w)', 'sqrt(e)sin(w)'],
            units = [None, None, None, None, 'days', 'BJD-2450000', None, None, None, 'm/s', None, None],
            dtype = [str, str, str, str, float, float, float, float, float, float, float, float])
    
    tab.write(dir+'/init_planets.txt', format = 'ascii.fixed_width_two_line', overwrite = True, delimiter = '|', delimiter_pad = ' ', bookend = True,
          formats = {'Period': '%11.6f', 'Tc': '%12.6f', 'Rp/Rs': '%7.4f', 'a/Rs': '%9.4f', 'cos(i)': '%6.4f', 'K': '%10.3f', 'sqrt(e)cos(w)': '%6.4f', 'sqrt(e)sin(w)': '%6.4f'},
          header_rows = ['name', 'unit'])


def create_init_ttvs(dir: str):

    input('Creating init_ttvs. If this was a mistake, press esc. Otherwise, enter to continue.')
    ...
    for x in data:
        if len(data[x]) < maxlen:
            data[x] += [np.nan]*(maxlen-len(data[x]))

    tab = Table(data)

    tab.write(dir+'/init_ttvs.txt', format = 'ascii.fixed_width_two_line', overwrite = True, delimiter = '|', delimiter_pad = ' ', bookend = True)


def generate_ld_grid(filter, filter_name):
    ...
    pickle.dump(d, open(os.path.dirname(os.path.realpath(__file__))+'/ld_grids/{0}_grid.p'.format(filter_name), 'wb'))
